...
const ORE_DISPONIBILE = Array.from({ length: 25 }, (_, i) => {
  ...
  const h = String(Math.floor(totalMinute / 60)).padStart(2, '0');
  const m = String(totalMinute % 60).padStart(2, '0');
  return `${h}:${m}`;
}); // 10:00 → 22:00, din 30 în 30
...
export default function PaginaRezervari() {
  ...
  // Interceptează click-uri pe linkuri când suntem la pasul 3
  useEffect(() => {
    if (pas !== 3) return;
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center px-6">
      ...
      <div className="relative bg-[#F5E6C8] rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={continuaRezervarea}
            className="w-full py-3.5 bg-[#1E1200] hover:bg-[#3B2507] text-[#F5E6C8] font-semibold rounded-2xl transition-all duration-300"
          >
            Continuă rezervarea
          </button>
          <button
            onClick={anuleazaRezervarea}
            className="w-full py-3.5 border-2 border-[#1E1200] text-[#1E1200] hover:bg-[#1E1200] hover:text-[#F5E6C8] font-semibold rounded-2xl transition-all duration-300"
          >
            Anulează rezervarea
          </button>
        </div>
      ...
      <>
        ...
        <main className="min-h-screen flex items-center justify-center px-6 pt-24 pb-16 bg-[#1C0F07]">
          <div className="bg-[#F5E6C8] rounded-3xl p-10 text-center max-w-md w-full">
            ...
            <button
              onClick={() => { setPas(1); setData(''); setOra(''); setForm({ nume: '', email: '', telefon: '', nr_persoane: 2 }); setSucces(false); setCalendarLuna(azi.getMonth()); setCalendarAn(azi.getFullYear()); }}
          ...
          <div className="bg-[#F5E6C8] rounded-3xl p-8">
            ...
            {pas === 1 && (
              <div>
                    ...
                    return (
                      <button
                        ...
                        className={`flex-shrink-0 flex flex-col items-center px-3 py-2.5 rounded-xl text-xs font-semibold border-2 transition-all duration-200 ${
                          selectata
                            ? 'bg-[#1E1200] border-[#1E1200] text-[#F5E6C8] scale-105'
                            : 'bg-transparent border-[#1E1200] text-[#1E1200] hover:scale-105'
                        }`}
                      >
                ...
                <div className="bg-[#1E1200]/8 border border-[#1E1200]/15 rounded-2xl p-4">
                  {/* Header lună */}
                  <div className="flex items-center justify-between mb-4">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg border-2 border-[#1E1200] text-[#1E1200] disabled:opacity-20 disabled:cursor-not-allowed hover:bg-[#1E1200] hover:text-[#F5E6C8] flex items-center justify-center transition-all"
                    >‹</button>
                    <span className="text-[#1E1200] font-semibold">
                      {LUNA_RO[calendarLuna]} {calendarAn}
                    </span>
                    <button
                      onClick={() => {
                        ...
                        if (`${nextAn}-${String(nextLuna + 1).padStart(2, '0')}` <= maxDataStr.slice(0, 7)) {
                          setCalendarLuna(nextLuna);
                          setCalendarAn(nextAn);
                        }
                      }}
                      ...
                      return (
                        <button
                          ...
                          className={`h-9 w-full rounded-lg text-sm font-medium transition-all duration-150 ${
                            ...
                            eAzi ? 'border-2 border-[#1E1200] text-[#1E1200] font-bold' :
            ...
            {pas === 2 && (
              <div>
                ...
                <div className="flex gap-3 mt-6">
                  <button
                    onClick={() => setPas(1)}
                    className="flex-1 py-4 border-2 border-[#1E1200] text-[#1E1200] hover:bg-[#1E1200] hover:text-[#F5E6C8] font-semibold rounded-2xl transition-all duration-300"
                  >
                    ← Înapoi
                  </button>
                  <button
              ...
              <div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <button
                        ...
                        className="w-11 h-11 rounded-xl border-2 border-[#1E1200] text-[#1E1200] hover:bg-[#1E1200] hover:text-[#F5E6C8] text-xl font-bold transition-all"
                      >−</button>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    disabled={!form.nume || !form.email || !form.telefon || loading}
                    className="flex-[2] py-4 bg-[#1E1200] hover:bg-[#3B2507] disabled:opacity-30 disabled:cursor-not-allowed text-[#F5E6C8] font-semibold text-lg rounded-2xl transition-all duration-300"
                  >
